...
we use it to fail fast on errors that shouldn’t occur during normal
...
func main() {
	...
	/*
		...
		errors. This is the only program on the site designed to
		panic.
	*/
	panic("a problem")
	...
	/*
		...
		handle. Here’s an example of panicking if we get an
...
program try to create a temp file, comment the first panic
out.

➜  gru git:(main) ✗ go run panic/panic.go
panic: a problem

goroutine 1 [running]:
main.main()
        /Users/lewinga/Developer/go/gru/panic/panic.go:7 +0x2c
exit status 2

Note that unlike some languages which use exceptions for handling
of many errors, in Go it is idiomatic to use error-indicating
