...
it concurrently from multiple goroutines, we add a Mutex to
synchronize access. Note that mutexes must not be copied,
so if this struct is passed around, it should be done by
pointer.
*/
...
func main() {
	...
	*/
	...
	/*
		...
		access the same Container, and two of them access the
		same counter.
	*/
	wg.Add(3)
...
➜  gru git:(main) ✗ go run mutexes/mutexes.go
map[a:20000 b:10000]
*/
